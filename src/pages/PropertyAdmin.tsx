...
import { CheckCircle2, Clock3, Eye, RefreshCcw, ShieldCheck, XCircle, AlertTriangle } from "lucide-react";
  ...
  return (
    <main className="min-h-screen bg-[#f7f9ff] text-slate-900 p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {error && <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{error} {!email && <a className="font-bold underline ml-2" href="/api/gbp?action=login">Sign in with approved Google account</a>}</div>}
        ...
        {loading ? <div className="rounded-2xl bg-white p-10 text-center">Loading property submissions…</div> : email && filtered.length === 0 ? <div className="rounded-2xl border bg-white p-10 text-center text-slate-500">No {filter.toLowerCase().replace(/_/g, " ")} properties.</div> : email && (
