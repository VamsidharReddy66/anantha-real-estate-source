...
function allowed(email) {
  const configured = process.env.PROPERTY_ADMIN_EMAILS || process.env.GBP_ADMIN_EMAILS || "info@anantharealestate.in";
  return typeof email === "string" && configured.split(",").map((s) => s.trim().toLowerCase()).filter(Boolean).includes(email.toLowerCase());
}
function unseal(value) {
  const b = Buffer.from(value, "base64");
  const key = Buffer.from(process.env.GBP_ENCRYPTION_KEY || "", "base64");
  if (key.length !== 32) throw new Error("Admin encryption key is invalid");
...
async function requireAdmin(req) {
  ...
  const origin = process.env.GBP_APP_ORIGIN || "https://www.anantharealestate.in";
  const key = `gbp:${hash(origin)}:session:${hash(sid)}`;
...
function origin() {
  return String(process.env.GBP_APP_ORIGIN || "https://www.anantharealestate.in").replace(/\/$/, "");
}
